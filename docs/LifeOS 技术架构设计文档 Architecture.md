...
- 本地数据库加密与备份遵循 ADR-0004：采用 Drift `NativeDatabase` 与 SQLite3MultipleCiphers、Keystore 托管随机数据库密钥、禁止系统自动备份，并提供用户主动的加密备份与原子恢复。在实现和真机验证完成前不得宣称“数据库已加密”。
...
本地数据库的加密决策已由 ADR-0004 确认，实施时必须在 vivo X100s 上验证加密引擎、明文迁移、备份恢复和性能。Diary 图片仍只受 App 私有目录与备份排除保护，尚未独立进行静态文件加密，因此产品不能宣称“所有本地数据均已加密”。
